...
@media (prefers-color-scheme: dark) {
    ...
    [data-testid="stNumberInput"] input { background-color: #444 !important; border: 1px solid #666 !important; }
...
@media (max-width: 600px) {
    ...
    [data-baseweb="select"] input { font-size: 1rem !important; }
}
...
if 'visit_counted' not in st.session_state:
    ...
    visit_data["visits"].append({"timestamp": datetime.now().strftime("%Y-%m-%dT%H:%M:%S")})
...
for key, default in [('selected_category','All'),('selected_subcategory','All'),('selected',[])]:
...
# ── recipe_options：從 recipes_df 來（乾淨 list，無重複）────────────────────
    ...
    if mode == ("基本模式" if lang == "中文" else "Basic Mode"):
        ...
        if st.button("隨機挑選" if lang == "中文" else "Random Pick", key="basic_random"):
            if not recipe_options:
                ...
                st.session_state.selected = random.sample(recipe_options, n)
                st.success(f"已隨機挑選 {n} 道餐點！" if lang == "中文" else f"Randomly selected {n} dishes!")
    else:
        num_dishes = st.number_input("選擇隨機餐點數量" if lang == "中文" else "Number of Dishes to Randomize", min_value=1, max_value=5, value=1, key="adv_num")
        filters = []
        for i in range(int(num_dishes)):
            ...
            cat = st.selectbox(
                ...
                ['All'] + sorted(recipes_df[cat_key].dropna().unique()), key=f"adv_cat_{i}"
            )
            if cat != 'All':
                styles = ['All'] + sorted(recipes_df[recipes_df[cat_key]==cat][subcat_key].dropna().unique())
            else:
                styles = ['All'] + sorted(recipes_df[subcat_key].dropna().unique())
            subcat = st.selectbox(f"風格 (第 {i+1} 道)" if lang == "中文" else f"Style (Dish {i+1})", styles, key=f"adv_sub_{i}")
        ...
        if st.button("隨機挑選" if lang == "中文" else "Random Pick", key="adv_random"):
            ...
            for idx, (cat, subcat) in enumerate(filters):
                tmp = list(set(recipe_options) - used)
                if cat != 'All':
                    tmp = [r for r in tmp if recipes_df.loc[recipes_df[display_col].str.replace(r'\*\*','',regex=True)==r, cat_key].values[0:1] == [cat]]
                if subcat != 'All':
                    tmp = [r for r in tmp if recipes_df.loc[recipes_df[display_col].str.replace(r'\*\*','',regex=True)==r, subcat_key].values[0:1] == [subcat]]
                if not tmp:
            ...
            if picked:
                ...
                st.success(f"已隨機挑選 {len(picked)} 道餐點！" if lang == "中文" else f"Randomly selected {len(picked)} dishes!")
            else:
...
with st.expander("🔍 進階篩選" if lang == "中文" else "🔍 Advanced Filters", expanded=False):
    ...
    if sel_cat != 'All':
        avail_styles = ['All'] + sorted(recipes_df[recipes_df[cat_key]==sel_cat][subcat_key].dropna().unique())
    else:
    ...
    sel_sub = st.selectbox(
        ...
        index=avail_styles.index(st.session_state.selected_subcategory) if st.session_state.selected_subcategory in avail_styles else 0
    )
...
if recipe_options:
    ...
    selected = st.multiselect(
        ...
        recipe_options, default=valid_selected
    )
...
if selected:
    ...
    for recipe in selected:
        ...
        if recipe_tools.empty:
            ...
            tool_display.columns = ["工具" if lang == "中文" else "Tool"]
            tool_display[opt_label] = recipe_tools["Optional"].apply(lambda x: "✓" if x else "") if "Optional" in recipe_tools.columns else ""
        ...
        if step_data.empty or not all(c in step_data.columns for c in required_cols):
            ...
            extra = ["CycleTime","Parallel"] if "CycleTime" in step_data.columns and "Parallel" in step_data.columns else []
    ...
    if lang == "中文":
        ...
        summary.columns   = ["食材","Unit","Optional","TotalAmount","數量","非必要"]
        ingredient_lines  = [f"{r['食材']}: {r['數量']}{r['Unit']}" + (" (非必要)" if r['非必要'] else "") for _, r in summary.iterrows()]
    else:
        ...
        ingredient_lines    = [f"{r['Ingredient']}: {r['Quantity']}{r['Unit']}" + (" (optional)" if r['Optional'] else "") for _, r in summary.iterrows()]
    ...
    st.markdown(f'<span style="margin-right:5px;">•</span><span style="font-size:1rem;font-weight:bold;">{"食材總和" if lang == "中文" else "Ingredients Summary"}</span>', unsafe_allow_html=True)
    st.code("\n".join(ingredient_lines))
    st.markdown(f'<span style="margin-right:5px;">•</span><span style="font-size:1rem;font-weight:bold;">{"工具總和" if lang == "中文" else "Tools Summary"}</span>', unsafe_allow_html=True)
    if tool_lines:
        st.code("\n".join(tool_lines))
    else:
        st.info("工具資料待補" if lang == "中文" else "Tool data to be added")
